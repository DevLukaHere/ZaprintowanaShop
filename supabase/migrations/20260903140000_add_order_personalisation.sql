-- P3: dane uroczystości (imiona, data, godzina, miejsca) przenosimy z karty produktu
-- do formularza zamówienia. Jeden komplet danych na całe zamówienie.
alter table public.orders
  add column if not exists personalisation jsonb;

comment on column public.orders.personalisation is
  'Dane uroczystości podane w formularzu zamówienia — imiona, data, godzina, miejsca, uwagi. Jeden komplet na zamówienie.';

-- Stara sygnatura zostaje usunięta, żeby nie było dwóch przeciążeń create_order.
...
create or replace function public.create_order(
  ...
  p_notes text,
  p_items jsonb,
  p_personalisation jsonb default null
)
returns uuid
language plpgsql
security definer
...
begin
  ...
  insert into public.orders (
    ...
    shipping_address, shipping_city, shipping_postcode, notes, personalisation
  )
  values (
    ...
    case when p_personalisation = '{}'::jsonb then null else p_personalisation end
  )
